...
function displayCurrentFolder() {
    ...
    // Собираем все элементы: сначала папки, затем программы
    const items = [];
    
    // Добавляем папки
    if (currentFolder.folders && currentFolder.folders.length > 0) {
        items.push(...currentFolder.folders.map(folder => ({ type: 'folder', data: folder })));
    }
    
    // Добавляем программы
    if (currentFolder.programs && currentFolder.programs.length > 0) {
        items.push(...currentFolder.programs.map(program => ({ type: 'program', data: program })));
    }
    
    // Если элементов нет, показываем сообщение
    if (items.length === 0) {
        itemsGrid.innerHTML = '<div class="no-items">В этой папке нет элементов</div>';
        return;
    }
...
// Отображение элементов (папок и программ) в едином списке
function renderItems(items) {
    ...
    items.forEach(item => {
        if (item.type === 'folder') {
            renderFolderItem(grid, item.data);
        } else if (item.type === 'program') {
            renderProgramItem(grid, item.data);
        }
    });
}

// Отображение элемента папки
function renderFolderItem(grid, folder) {
    ...
    folderCard.innerHTML = `
        ...
        <div class="item-content">
            ...
            ${folder.description ? `<p class="item-description">${folder.description}</p>` : ''}
            ${folder.programs ? `<div class="item-info"><i class="fas fa-dumbbell"></i> ${folder.programs.length} программ</div>` : ''}
        </div>
...
// Отображение элемента программы
function renderProgramItem(grid, program) {
    ...
    card.innerHTML = `
        ...
        <div class="program-card-footer">
            <button class="action-btn program-select-btn" data-program-id="${program.id}">
    ...
    if (selectBtn) {
        selectBtn.addEventListener('click', async (e) => {
            e.stopPropagation();
            const programId = selectBtn.dataset.programId;
            let selectedProgram = findProgramById(programId);
            if (!selectedProgram) {
                // Ищем программу в текущей папке
                const currentFolder = getCurrentFolder();
                if (currentFolder.programs) {
                    selectedProgram = currentFolder.programs.find(p => p.id === programId);
                }
            }
            if (selectedProgram) {
